...
namespace EEngine.EEngine
{
    public class Map
    {
        ...
        public Vector2 Map_Array_Position { get; private set; } = Vector2.Zero();

        public Map(Vector2 Position, Vector2 Scale, Tiles Tile, Vector2 Map_Array_Position)
        {
            this.Position = Position;
            this.Scale = Scale;
            this.Tile = new Tiles(Scale, Tile.Sprite, Tile.Tag, Tile.ShortTag, false);
            this.Map_Array_Position = Map_Array_Position;
        ...
        public Map(Vector2 Tile_Scale, string[,] MapArray)
        {
            ...
            EEngine.InitializeMapArray(MapArray.GetLength(0));
            for (int i = 0; i < MapArray.GetLength(0); i++)
            {
                for (int j = 0; j < MapArray.GetLength(1); j++)
                {
                    new Map(new Vector2(j * Tile_Scale.X + StartPosition.X, i * Tile_Scale.Y + StartPosition.Y), Tile_Scale, EEngine.GetTile(MapArray[i,j]), new Vector2(j, i));
                }
            }
            EEngine.Loaded();
        }
        public Map(float Scale, Vector2 Tile_Scale, string[,] MapArray)
        {
            Vector2 StartPosition = Vector2.Zero();
            ...
            for (int i = 0; i < MapArray.GetLength(0); i++)
            {
                for (int j = 0; j < MapArray.GetLength(1); j++)
                {
                    new Map(new Vector2(j * Tile_Scale_2.X + StartPosition.X, i * Tile_Scale_2.Y + StartPosition.Y), Tile_Scale_2, EEngine.GetTile(MapArray[i, j]), new Vector2(j, i));
                }
            }
            EEngine.Loaded();
        }
